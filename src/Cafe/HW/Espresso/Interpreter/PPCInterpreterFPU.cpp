...
const int ieee_double_m_bits = 52; // mantUSER bits
...
ATTR_MS_ABI double frsqrte_espresso(double input)
{
	...
	if (e == 0x7FF)
	{
		...
		// result is NaN with same sign and same mantUSER (todo: verify)
		return *(double*)&x;
	}
	...
	// get index (lsb of exponent, remaining bits of mantUSER)
...
{
	// based on testing we know that fres uses only the first 15 bits of the mantUSER
	// seee eeee eeee mmmm mmmm mmmm mmmx xxxx ....		(s = sign, e = exponent, m = mantUSER, x = not used)
	// the mantUSER bits are interpreted as following:
